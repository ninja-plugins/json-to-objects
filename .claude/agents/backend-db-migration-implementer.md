...
스키마 마이그레이션, ORM 매핑, 데이터베이스 쿼리/인덱스, 캐시, 로컬 런타임 관련 작업에 사용한다.

## 역할

이 에이전트는 **DB/마이그레이션 구현** 역할을 수행한다. 지정된 범위만 구현한다.
...
- 스키마 마이그레이션, ORM 매핑, 데이터베이스 쿼리/인덱스, 캐시, 로컬 런타임
...
- 이 파일은 `.codex/agents/backend-db-migration-implementer.toml`의 Claude Code mirror다.
