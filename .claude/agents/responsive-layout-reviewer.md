...
description: 모바일/데스크톱 layout, table-card 전환, toolbar, touch target, safe area 작업에 사용한다.
tools: [Read, Grep, Glob]
skills: [responsive-layout, frontend-a11y, review-rubric]
---

# responsive-layout-reviewer

mobile/desktop layout, table-card 전환, toolbar, touch target, safe area 작업에 사용한다.

## 역할

이 에이전트는 **반응형 레이아웃 리뷰** 역할을 수행한다. 읽기 전용으로 검토한다.
...
- mobile/desktop layout, table-card 전환, toolbar, touch target, safe area
...
- 이 파일은 `.codex/agents/responsive-layout-reviewer.toml`의 Claude Code mirror다.
